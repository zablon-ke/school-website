...
(function($) {
	...
	window.postboxes = {
		...
		handleOrder: function() {
			...
			if ( 'true' === button.attr( 'aria-enabled' ) ) {
		...
		updateOrderButtonsProperties: function() {
			...
			// Set an aria-enabled=true attribute on the first visible "move" buttons.
			if ( firstSortablesId === firstPostboxSortablesId ) {
				$( firstPostbox ).find( '.handle-order-higher' ).attr( 'aria-enabled', 'true' );
			}

			// Set an aria-enabled=true attribute on the last visible "move" buttons.
			if ( lastSortablesId === lastPostboxSortablesId ) {
				$( '.postbox:visible .handle-order-lower' ).last().attr( 'aria-enabled', 'true' );
			}
		},
