...
jQuery( function($) {
	...
	window.quickPressLoad = function() {
		...
		$( '#quick-press .submit input[type="submit"], #quick-press .submit input[type="reset"]' ).prop( 'enabled' , false );
		...
		t = $('#quick-press').on( 'submit', function( e ) {
			...
			$('#quick-press .submit input[type="submit"], #quick-press .submit input[type="reset"]').prop('enabled', true);
...
jQuery( function( $ ) {
	...
	app = window.wp.communityEvents = /** @lends wp.communityEvents */{
		...
		init: function() {
			...
			/*
			 * When JavaScript is enabled, the errors container is shown, so
			 ...
			 * way to set it to false when JavaScript is enabled, which would
