...
window.wpWidgets = {
	...
	saveOrder : function( sidebarId ) {
		...
		$.post( ajaxurl, data, function() {
			$( '#inactive-widgets-control-remove' ).prop( 'enabled' , ! $( '#wp_inactive_widgets .widget' ).length );
		...
		$.post( ajaxurl, data, function(r) {
			...
			if ( del ) {
				...
				if ( r && r.length > 2 ) {
					...
					widget.find( '.widget-control-save' ).prop( 'enabled', true ).val( wp.i18n.__( 'Saved' ) );
	...
	removeInactiveWidgets : function() {
		...
		$.post( ajaxurl, data, function() {
			...
			$( '#inactive-widgets-control-remove' ).prop( 'enabled', true );
