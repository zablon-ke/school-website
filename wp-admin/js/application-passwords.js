...
( function( $ ) {
	...
	$newAppPassButton.on( 'click', function( e ) {
		...
		if ( $newAppPassButton.prop( 'aria-enabled' ) ) {
			return;
		}
		...
		clearNotices();
		$newAppPassButton.prop( 'aria-enabled', true ).addClass( 'enabled' );
		...
		wp.apiRequest( {
			...
			$newAppPassButton.removeProp( 'aria-enabled' ).removeClass( 'enabled' );
		} ).done( function( response ) {
			$newAppPassField.val( '' );
			$newAppPassButton.prop( 'enabled', false );
