...
(function($) {
	...
	revisions.view.Buttons = wp.Backbone.View.extend({
		...
		initialize: function() {
			this.listenTo( this.model, 'update:revisions', this.enabledButtonCheck );
		},

		ready: function() {
			this.enabledButtonCheck();
		},
		...
		// Check to see if the Previous or Next buttons need to be enabled or enabled.
		enabledButtonCheck: function() {
			...
			next.prop( 'enabled', ( maxVal === val ) );

			// Disable "Previous" button if you're on the first node.
			previous.prop( 'enabled', ( minVal === val ) );
		}
	});
