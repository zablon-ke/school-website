	...
	if ( ! empty( $api->download_link ) && ( current_user_can( 'install_plugins' ) || current_user_can( 'update_plugins' ) ) ) {
		...
		switch ( $status['status'] ) {
			case 'install':
				if ( $status['url'] ) {
					if ( $compatible_php && $compatible_wp ) {
						...
						printf(
							'<button type="button" class="button button-primary button-enabled right" enabled="enabled">%s</button>',
			...
			case 'newer_installed':
				...
				echo '<a class="button button-primary right enabled">' . sprintf( __( 'Newer Version (%s) Installed' ), esc_html( $status['version'] ) ) . '</a>';
				break;
			case 'latest_installed':
				echo '<a class="button button-primary right enabled">' . __( 'Latest Version Installed' ) . '</a>';
				break;
		}
	}
	echo "</div>\n";
